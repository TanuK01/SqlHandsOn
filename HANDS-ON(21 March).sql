...
select distinct count(name) from Salesman
...
--select order_date, MAX(purch_amt) from Orders where order_date='2012-09-10' group by customer_id
...
--select salesman_id, MAX(purch_amt) from Orders group by salesman_id having order_date='2012-08-17'

--13.	write a SQL query to find the salesperson and customer who belongs to same city. Return Salesman, cust_name and city.
select Salesman.Salesman_id, cust_name, Customer.city from Salesman full outer join Customer on Salesman.city = Customer.city
...
select Salesman_id, name, city, commision from Salesman where commision>0.12
...
select cust_name, Customer.city, grade, Customer.Salesman_id, Salesman.city from Customer full outer join Salesman 
on Salesman.name = Customer.cust_name order by customer_id
