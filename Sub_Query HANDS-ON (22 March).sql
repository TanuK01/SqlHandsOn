...
select order_no, purch_amt, order_date, salesman_id, customer_id from Orders where salesman_id 
...
in (select salesman_id from Salesman where city='London')
...
select * from Orders where salesman_id in (select distinct salesman_id from Customer where customer_id=3007)
...
select * from Orders where purch_amt>(select AVG(purch_amt) as 'Avg_Purch' from Orders where order_date='2012-10-10')
...
select order_date, SUM(purch_amt) from Orders a 
group by order_date 
having sum(purch_amt)> (select 1000.00 + max(purch_amt) from Orders b where a.order_date = b.order_date)
