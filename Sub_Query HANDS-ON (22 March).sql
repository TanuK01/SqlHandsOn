...
select order_no, purch_amt, order_date, salesman_id, customer_id from Orders where customer_id 
in (select salesman_id from Salesman where name='Paul Adam')
...
select * from Orders where purch_amt>(select AVG(purch_amt) from Orders where order_date='2012-10-10')
...
select salesman_id, cust_name from Customer where salesman_id 
in(select salesman_id from Customer group by salesman_id having COUNT(salesman_id)>1)
...
--select order_date, SUM(purch_amt) from Orders group by order_date not in() 
--select order_date, SUM(purch_amt) from Orders group by order_date where order_date 
--in (select order_date, MAX(purch_amt) from Orders group by order_date)
...
in (select Count(customer_id) from Customer group by city having city='London')

--10. write a SQL query to find the salespeople who deal multiple customers. Return salesman_id, name, city and commission
select salesman_id, name, city, commision from Salesman where salesman_id 
in(select salesman_id from Customer group by salesman_id having COUNT(salesman_id)>1)